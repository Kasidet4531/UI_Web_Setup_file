...
### Action-Oriented Summary Cards (Default UI Layout)

To maximize day-to-day operational efficiency and allow users to immediately focus on actionable requests, the Dashboard utilizes 4 interactive metric cards that act as instant quick-filters:

```text
1. My Open Requests (Active requests assigned to or submitted by the current user)
2. Waiting for Setup (Submitted requests awaiting setup owner pickup)
3. Setup In Progress (Requests currently in engineering workflow)
4. Overdue Actions (Requests past due date and still pending completion)
```

> [!NOTE]
> Clicking any metric card dynamically filters the Request List Table to display the corresponding subset of requests. Additional metrics such as Total Requests, Completed, and PSF Created are accessible via filter dropdowns and the All Requests table views.
...
### Lifecycle Quick Action Shortcuts

In addition to the manual status dropdown, the Request Detail Action Center provides contextual one-click action shortcuts:
1. **Submitted Stage**: Setup Owner sees **"Accept & Start Setup"** -> transitions to `SETUP_IN_PROGRESS` and assigns setup owner.
2. **Setup In Progress Stage**: Setup Owner sees **"Mark PSF Created"** -> transitions to `PSF_CREATED`, unlocking the PSF Created Information section for the requester.
3. **PSF Created Stage**: Setup Owner, Requester, or Admin sees **"Mark Request Completed"** -> transitions to `COMPLETED` once all verification is completed.
...
- Provide an interactive Drag & Drop upload zone supporting common engineering file formats (`.xlsx`, `.pdf`, `.zip`, `.psf`, `.job`, `.json`).
- Render uploaded files as identifiable file chips showing icon, filename, and formatted file size.
- Include one-click remove buttons for active editing and simulated download / preview capability in read-only mode.
- Clearly indicate empty / locked states for uncompleted workflow stages.
